...
const resolvers = {
  ...
  Mutation: {
    ...
    updateUser: async (_, { id, input }, context) => {
      ...
      if (input.role && input.role !== authUser.role && authUser.role !== 'admin') {
   ...
   /* updateAIPrediction: async (_, { id, conditions, riskLevel }, { user }) => {
    ...
    createMotivation: async (_, { input }, { user }) => {
      ...
      updateMotivation: async (_, { id, content }, { user }) => {
        ...
        if (!user || user.role !== 'nurse' || user.id !== motivation.NurseID.toString()) {
     ...
     recordVitalSigns: async (_, { input }, { user }) => {
        ...
        if (user.role === 'patient' && user.id !== input.PatientID)  {
          throw new AuthenticationError('Patients can only record their own vital signs');
        }
      ...
      updateVitalSigns: async (_, { id, input }, { user }) => {
        ...
        const updates = {
          ...
          RespiratoryRate: input.respiratoryRate ?? existingRecord.RespiratoryRate,
